    ...
    <ResponsiveContainer width="100%" height={Math.max(200, data.length * 36)}>
      <BarChart data={data} layout="vertical" margin={{ left: 80, right: 20, top: 5, bottom: 5 }}>
        ...
        <Tooltip content={({ active, payload }) => {
          ...
          return <div style={tooltipStyle}><div style={{ color: "var(--muted)", marginBottom: 2 }}>{d.name}</div><div style={{ fontSize: 12 }}>Cost: {formatAed(d.cost * 100)}</div><div style={{ fontSize: 12 }}>Value: {formatAed(d.value * 100)}</div></div>;
        }} />
