...
export default function RentalIncomeChart({ properties }: Props) {
  const data = useMemo(() => {
    return properties
      .filter((p) => p.subcategory !== "off_plan")
      .map((p) => ({ name: p.name, fils: netAnnualRentFils(p) }))
      .filter((d) => d.fils !== null && d.fils > 0)
      .sort((a, b) => b.fils! - a.fils!);
  }, [properties]);

  if (data.length === 0) return <p className="muted">No rental income data available.</p>;
  ...
  return (
    <ResponsiveContainer width="100%" height={Math.max(120, data.length * 34)}>
      <BarChart data={data} layout="vertical" margin={{ left: 90, right: 20, top: 4, bottom: 4 }}>
        ...
        />
        <Bar dataKey="fils" fill="#4f9cf9" radius={[0, 4, 4, 0]} />
      </BarChart>
    </ResponsiveContainer>
  );
}
